...
/** A candidate spelling of the clicked text, plus a ranking penalty (> 0 when it
 *  was only reachable through a lossy normalization — see below). */
 ...
 *
 * Each variant carries a `penalty`: 0 for the safe normalizations, but +1 for
 * the *full* emphatic collapse, which removes sokuon/長音 that may be grammatical
 * (e.g. わかってる → わかてる drops the te-form っ, letting it read as 分かつ's
 * potential form). The penalty keeps such coincidental parses from outranking
 * the natural one on inflection-chain length.
 *
 * Ports the feasible subset of Yomitan's `ja` textPreprocessors, plus two
 * manga-oriented additions (chōon expansion, 旧字体→新字体). Omitted:
 * alphabeticToHiragana (romaji input — irrelevant to OCR) and
 * normalizeRadicalCharacters (needs a large external mapping table).
 */
export function generateTextVariantsWithPenalty(text: string): TextVariant[] {
  // Safe, penalty-free preprocessors compose first.
  const safe: ((s: string) => string[])[] = [
    (s) => [s, convertHalfWidthKanaToFullWidth(s)],
    (s) => [s, normalizeCombiningCharacters(s)],
    (s) => [s, normalizeCJKCompatibilityCharacters(s)],
    (s) => [s, normalizeEnclosedCJKCharacters(s)],
    (s) => [s, convertKyuujitaiToShinjitai(s)],
  ...
  // text -> minimum penalty seen for that spelling.
  let variants = new Map<string, number>([[text, 0]]);
  for (const preprocess of safe) {
    const next = new Map<string, number>();
    for (const [v, penalty] of variants) {
      for (const out of preprocess(v)) {
        const cur = next.get(out);
        if (cur === undefined || penalty < cur) next.set(out, penalty);
      }
    }
    variants = next;
  }

  // Emphatic collapse: the partial form keeps one of a run (safe); the full form
  // strips them entirely (penalized).
  const withEmphatic = new Map<string, number>(variants);
  const bump = (t: string, p: number) => {
    const cur = withEmphatic.get(t);
    if (cur === undefined || p < cur) withEmphatic.set(t, p);
  };
  for (const [v, penalty] of variants) {
    bump(collapseEmphaticSequences(v, false), penalty);
    bump(collapseEmphaticSequences(v, true), penalty + 1);
  }

  return [...withEmphatic].map(([t, penalty]) => ({ text: t, penalty }));
}

/** Variant spellings only (penalty discarded) — convenience for callers that
 *  don't rank, and for tests. */
export function generateTextVariants(text: string): string[] {
  return generateTextVariantsWithPenalty(text).map((v) => v.text);
}
